...
#네이버리뷰 테이블에 넣기
f=open('naver_review.csv','rt', encoding='UTF8')
data=csv.reader(f)
next(data)

for row in data:
    #번호(int)-영화제목-리뷰내용-리뷰별점(int)-총점(int)-날짜(date)
...
#코비스 테이블에 넣기
f1=open('kobis_rank.csv','rt', encoding='UTF8')
k_rank=csv.reader(f1)
next(k_rank)

f2=open('kobis_movie.csv','rt', encoding='UTF8')
...
for row1,row2 in zip(k_rank,k_movie):
    ...
    {0}, '{1}', '{2}', {3},'{4}','{5}','{6}','{7}','{8}'
    )'''.format(row1[0],row2[0],row1[1],row1[3],row2[2],row1[4],row2[3],row2[4],row2[1])
    curs.execute(query)
