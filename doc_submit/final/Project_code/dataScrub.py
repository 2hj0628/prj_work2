...
try:    
    ...
    # RuntimeError: main thread is not in main loop self.tk.call('image', 'delete', self.name) 발생시 
    # Agg는 파일에만 쓸 수 있는 비대화형 백엔드
    # matplotlib.use('Agg') 그래프 안그려짐,저장으로 가야함.
...
def modelFit(X_train, y_train):
    ...
    print('\n┌──────────────────────────────────────────────────────────────────────┐')
    print('│ >> 결과확인 : Overfitting (과대적합), 에크모를 2회로 조정 후 재실행  │')
    print('└──────────────────────────────────────────────────────────────────────┘')
    time.sleep(3)
